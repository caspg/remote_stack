module Scrapers
  module WeWorkRemotely
    WWR_ORIGIN_NAME = 'We Work Remotely'.freeze
    ...
    class << self
      ...
      def parse_rss_feed_items(last_origin_id)
        ::Scrapers::WeWorkRemotely::ParseRss.new(last_origin_id: last_origin_id).call
      end

      def scrap_and_prepare_data(rss_feed_item)
        {
          # TODO(kacper): do something with it
          origin_name: WWR_ORIGIN_NAME,
      ...
      def scrap_job_details(rss_feed_item)
        ...
        ::Scrapers::WeWorkRemotely::ScrapJobDetails.new(rss_feed_item: rss_feed_item).call
      end
